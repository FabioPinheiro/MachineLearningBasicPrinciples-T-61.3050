...
%The pseudocode for k-NN in Algorithm~\ref{alg:KNN}, is from Statistical Methods in Data Mining Lecture Slides \cite{CA}.
...
\midrule
Type (With out the data scaled) & $0.953$\\
Type (With the data scaled) & $0.994$\\
\hline
Quality (With out the data scaled) & $0.428$\\
Quality (With the data scaled) & $0.634$\\
\end{tabular}
\end{table}

\subsubsection*{Confusion matrix for the wine quality}
...
Type & $0.996$\\
Quality & $0.513$\\
Quality using type & $0.518$\\
\end{tabular}
\end{table}


\subsubsection*{Confusion matrix for the wine quality using type}
\begin{table}[H]
